...
public static class SmartSerializer
{
	...
	public static void DeserializeInto<T>(T obj, byte[] data)
	{
		...
		SerializerData sd = ReadSerializerData(br);
		ReadHeader(br);
		DeserializeIntoInternal(obj, typeof(T), br, sd);
	}

	private static void SerializeInternal<T>(T o, BinaryWriter bw, SerializerData sd) where T : class
	{
		ClearCache();
		SerializeInternal(typeof(T), o, bw, sd);
	}

	private static void ClearCache()
	{
		_fields_cache.Clear();
		_fields_hashes_cache.Clear();
		_string_hashes.Clear();
	}

	private static List<FieldInfo> GetSerializableFieldsForType(Type obj_type)
	{
		if (_fields_cache.TryGetValue(obj_type, out var value))
		{
			return value;
		}
		value = new List<FieldInfo>();
		FieldInfo[] fields = obj_type.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
		foreach (FieldInfo fieldInfo in fields)
		{
			if (((fieldInfo.IsPublic && !fieldInfo.IsNotSerialized) || Attribute.IsDefined(fieldInfo, TYPE_SERIALIZED_FIELD, inherit: true) || Attribute.IsDefined(fieldInfo, TYPE_SMART_SERIALIZE, inherit: true)) && !Attribute.IsDefined(fieldInfo, TYPE_SMART_DONT_SERIALIZE) && !obj_type.IsSubclassOf(TYPE_COMPONENT) && !obj_type.IsSubclassOf(TYPE_MONOBEHAVIOUR))
			{
				value.Add(fieldInfo);
			}
		}
		_fields_cache.Add(obj_type, value);
		return value;
	}

	private static Dictionary<int, FieldInfo> GetFieldsHashesForType(Type obj_type)
	{
		if (obj_type != null && _fields_hashes_cache.TryGetValue(obj_type, out var value))
		{
			return value;
		}
		...
		foreach (FieldInfo item in GetSerializableFieldsForType(obj_type))
		{
			value.Add(item.Name.GetStableHashCode(), item);
		}
		_fields_hashes_cache.Add(obj_type, value);
		return value;
	}

	private static void SerializeInternal(Type obj_type, object o, BinaryWriter bw, SerializerData sd)
	{
		if (o == null)
		{
			bw.Write(-1);
		...
		List<FieldInfo> serializableFieldsForType = GetSerializableFieldsForType(obj_type);
		if (TYPE_INTERFACE.IsAssignableFrom(obj_type))
		{
	...
	private static object DeserializeInternal(Type obj_type, BinaryReader br, SerializerData sd)
	{
		return DeserializeIntoInternal((obj_type == null) ? null : Activator.CreateInstance(obj_type), obj_type, br, sd);
	}

	private static object DeserializeIntoInternal(object obj, Type obj_type, BinaryReader br, SerializerData sd)
	{
		Dictionary<int, FieldInfo> fieldsHashesForType = GetFieldsHashesForType(obj_type);
		...
		for (int i = 0; i < num; i++)
		{
			...
			catch (MissingMethodException)
			{
				if (value != null)
				{
					Debug.LogError("Error deserializing object: " + value.FieldType?.ToString() + ", name = " + value.Name);
				}
				throw;
			}
			if (value != null)
			...
			Debug.LogError("Couldn't find a field to deserialize, obj_type = " + obj_type?.ToString() + ", field_type = " + ((obj2 == null) ? "null" : obj2.GetType().ToString()) + ", hash_size = " + fieldsHashesForType.Count);
		}
		if (obj_type != null && TYPE_INTERFACE.IsAssignableFrom(obj_type))
		{
	...
	private static object DeserializeObject(Type obj_type, BinaryReader br, SerializerData sd)
	{
		...
		case FieldType.String_Empty:
			return string.Empty;
		case FieldType.String_Indexed:
		...
		case FieldType.Array:
		{
			...
			Type type2 = ((obj_type == null) ? typeof(object) : obj_type.GetElementType());
		...
		{
			int num = br.ReadInt32();
			Type type = ((obj_type == null) ? typeof(object) : obj_type.GetGenericArguments()[0]);
		...
		case FieldType.SmartSerialized:
			return DeserializeInternal(obj_type, br, sd);
		default:
	...
	private static string ReadStringFromStream(BinaryReader br, bool encrypt = false)
	{
		...
		while (num2 > 0)
		{
			int num3 = ((num2 <= _buffer.Length) ? num2 : _buffer.Length);
			num2 -= num3;
			long num4 = br.Read(_buffer, 0, num3);
			if (encrypt)
			{
	...
	{
		if (_string_hashes.TryGetValue(str, out var value))
		{
			return value;
		}
		int num = 5381;
