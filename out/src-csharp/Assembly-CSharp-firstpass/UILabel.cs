...
public class UILabel : UIWidget
{
	...
	public void ProcessText(bool legacyMode = false, bool full = true)
	{
		...
		if (mFinalFontSize > 0)
		{
			...
			while (num3 > 0)
			{
				...
				bool flag3 = NGUIText.WrapText(printedText, out mProcessedText, keepCharCount: false, wrapLineColors: false, mOverflowEllipsis);
	...
	public override void OnFill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		...
		if (NGUIText.symbolStyle == NGUIText.SymbolStyle.NoEffect)
		{
			...
			{
				Color color2 = cols[i];
				if (color2.r == 1f && color2.g == 0f && color2.b == 1f && color2.a == 0f)
				{
					cols[i] = new Color(1f, 1f, 1f, finalAlpha);
				}
			}
		}
		if (onPostFill != null)
