...
public static class Utils
{
	public static float EaseFromTo(float start, float end, float value, EaseType type = EaseType.EaseInOut)
	{
		...
		return type switch
		{
			EaseType.EaseInOut => Mathf.Lerp(start, end, value * value * (3f - 2f * value)), 
			EaseType.EaseOut => Mathf.Lerp(start, end, Mathf.Sin(value * (float)Math.PI * 0.5f)), 
			EaseType.EaseIn => Mathf.Lerp(start, end, 1f - Mathf.Cos(value * (float)Math.PI * 0.5f)), 
