...
public class ProCamera2DForwardFocus : BasePC2D, IPreMover
{
	...
	{
		float num = Vector3H(ProCamera2D.TargetsMidPoint) - Vector3H(ProCamera2D.PreviousTargetsMidPoint);
		num = ((!(Mathf.Abs(num) < MovementThreshold.x)) ? (num / deltaTime) : 0f);
		float num2 = Vector3V(ProCamera2D.TargetsMidPoint) - Vector3V(ProCamera2D.PreviousTargetsMidPoint);
		num2 = ((!(Mathf.Abs(num2) < MovementThreshold.y)) ? (num2 / deltaTime) : 0f);
		if (Progressive)
		{
			num = Mathf.Clamp(num * SpeedMultiplier, (0f - LeftFocus) * ProCamera2D.ScreenSizeInWorldCoordinates.x, RightFocus * ProCamera2D.ScreenSizeInWorldCoordinates.x);
			num2 = Mathf.Clamp(num2 * SpeedMultiplier, (0f - BottomFocus) * ProCamera2D.ScreenSizeInWorldCoordinates.y, TopFocus * ProCamera2D.ScreenSizeInWorldCoordinates.y);
		...
		{
			num = ((Mathf.Abs(num) < 0.001f) ? 0f : (((num < 0f) ? (0f - LeftFocus) : RightFocus) * ProCamera2D.ScreenSizeInWorldCoordinates.x));
			num2 = ((Mathf.Abs(num2) < 0.001f) ? 0f : (((num2 < 0f) ? (0f - BottomFocus) : TopFocus) * ProCamera2D.ScreenSizeInWorldCoordinates.y));
		}
		else
		{
			bool flag;
			...
			{
				_targetHVel = ((num < 0f) ? (0f - LeftFocus) : RightFocus) * ProCamera2D.ScreenSizeInWorldCoordinates.x;
			}
			num = _targetHVel;
			bool flag2;
			...
			{
				_targetVVel = ((num2 < 0f) ? (0f - BottomFocus) : TopFocus) * ProCamera2D.ScreenSizeInWorldCoordinates.y;
			}
			num2 = _targetVVel;
		}
		num = Mathf.Clamp(num, (0f - LeftFocus) * ProCamera2D.ScreenSizeInWorldCoordinates.x, RightFocus * ProCamera2D.ScreenSizeInWorldCoordinates.x);
		num2 = Mathf.Clamp(num2, (0f - BottomFocus) * ProCamera2D.ScreenSizeInWorldCoordinates.y, TopFocus * ProCamera2D.ScreenSizeInWorldCoordinates.y);
