...
public class ProCamera2DShake : BasePC2D
{
	...
	protected override void Awake()
	{
		...
		{
			Transform shakeParent = (ProCamera2D.transform.parent = new GameObject("ProCamera2DShakeContainer").transform);
	...
	private IEnumerator ApplyShakeTimedRoutine(Vector2 shake, Quaternion rotation, float duration, bool ignoreTimeScale = false)
	{
		...
		while (duration > 0f)
		{
			duration = ((!ignoreTimeScale) ? (duration - ProCamera2D.DeltaTime) : (duration - Time.unscaledDeltaTime));
	...
	private IEnumerator ConstantShakeRoutine(float intensity)
	{
		while (_isConstantShaking)
		{
			if (ProCamera2D.DeltaTime > 0f)
			{
				Vector3 vector = Utils.GetVectorsSum(_constantShakePositions) / _constantShakePositions.Length;
				_constantShakePosition.Set(Utils.SmoothApproach(_constantShakePosition.x, vector.x, vector.x, intensity, ProCamera2D.DeltaTime), Utils.SmoothApproach(_constantShakePosition.y, vector.y, vector.y, intensity, ProCamera2D.DeltaTime), Utils.SmoothApproach(_constantShakePosition.z, vector.z, vector.z, intensity, ProCamera2D.DeltaTime));
				_influences.Add(_constantShakePosition);
			}
			yield return ProCamera2D.GetYield();
		}
