...
public class ProCamera2DRails : BasePC2D, IPreMover
{
	...
	protected override void Awake()
	{
		...
		for (int i = 0; i < CameraTargets.Count; i++)
		{
			...
			ProCamera2D.AddCameraTarget(transform).TargetOffset = CameraTargets[i].TargetOffset;
		}
	...
	private void Step()
	{
		...
		for (int i = 0; i < CameraTargets.Count; i++)
		{
			...
			case FollowMode.HorizontalAxis:
				pos = VectorHVD(Vector3H(CameraTargets[i].TargetPosition) * CameraTargets[i].TargetInfluenceH, Vector3V(ProCamera2D.LocalPosition), 0f);
				break;
			case FollowMode.VerticalAxis:
				pos = VectorHVD(Vector3H(ProCamera2D.LocalPosition), Vector3V(CameraTargets[i].TargetPosition) * CameraTargets[i].TargetInfluenceV, 0f);
				break;
	...
	public void RemoveRailsTarget(Transform targetTransform)
	{
		...
		if (railsTarget != null)
		{
			...
			ProCamera2D.RemoveCameraTarget(_cameraTargetsOnRails[railsTarget]);
		}
	}
	...
	public void EnableTargets(float transitionDuration = 0f)
	{
		for (int i = 0; i < _tempCameraTargets.Count; i++)
		{
			...
			ProCamera2D.AddCameraTarget(_cameraTargetsOnRails[CameraTargets[i]], 1f, 1f, transitionDuration);
		}
