...
public class GridGraph : NavGraph, IUpdatableGraph, IRaycastableGraph
{
	...
	public override NNInfo GetNearestForce(Vector3 position, NNConstraint constraint)
	{
		...
		float num7 = (constraint.constrainDistance ? AstarPath.active.maxNearestNodeDistance : float.PositiveInfinity);
	...
	public virtual bool IsValidConnection(GridNode n1, GridNode n2)
	{
		...
		if (!(maxClimb <= 0f))
		{
			return (float)Math.Abs(n1.position[maxClimbAxis] - n2.position[maxClimbAxis]) <= maxClimb * 1000f;
		}
		return true;
	}
	...
	public void UpdateArea(GraphUpdateObject o)
	{
		...
		GetBoundsMinMax(o.bounds, inverseMatrix, out var min, out var max);
		int xmin = Mathf.RoundToInt(min.x - 0.5f);
		int xmax = Mathf.RoundToInt(max.x - 0.5f);
		int ymin = Mathf.RoundToInt(min.z - 0.5f);
		int ymax = Mathf.RoundToInt(max.z - 0.5f);
	...
	public bool Linecast(Vector3 _a, Vector3 _b, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace)
	{
		...
		while (gridNodeBase3.NodeInGridIndex != gridNodeBase2.NodeInGridIndex)
		{
			...
			int num4 = ((CrossMagnitude(a, vector3 - vector) + num < 0f) ? num3 : num2);
			GridNodeBase neighbourAlongDirection = GetNeighbourAlongDirection(gridNodeBase3, num4);
			if (neighbourAlongDirection != null)
			{
				gridNodeBase3 = neighbourAlongDirection;
				continue;
			}
			Vector2 vector4 = vector3 + new Vector2(neighbourXOffsets[num4], neighbourZOffsets[num4]) * 0.5f;
			Vector2 vector5 = ((neighbourXOffsets[num4] != 0) ? new Vector2(0f, 1f) : new Vector2(1f, 0f));
			...
			Vector3 point2 = new Vector3(vector4.x + 0.5f, vector7.y, vector4.y + 0.5f);
			hit.point = matrix.MultiplyPoint3x4(point);
			hit.tangentOrigin = matrix.MultiplyPoint3x4(point2);
	...
	public override void FastClearNodes()
	{
		nodes = new GridNode[0];
	}
}
