...
public class StartEndModifier : PathModifier
{
	...
	public override void Apply(Path _p)
	{
		if (_p is ABPath aBPath && aBPath.vectorPath.Count != 0)
		{
			...
			bool forceAddPoint;
			Vector3 vector = Snap(aBPath, exactStartPoint, start: true, out forceAddPoint);
			bool forceAddPoint2;
			Vector3 vector2 = Snap(aBPath, exactEndPoint, start: false, out forceAddPoint2);
	...
	private Vector3 Snap(ABPath path, Exactness mode, bool start, out bool forceAddPoint)
	{
		...
		case Exactness.ClosestOnNode:
			return GetClampedPoint(vector, start ? path.startPoint : path.endPoint, graphNode);
		...
		case Exactness.NodeConnection:
		{
			...
			case Exactness.Interpolate:
			{
				Vector3 clampedPoint = GetClampedPoint(vector, vector2, graphNode);
			...
			case Exactness.NodeConnection:
			{
				...
				connectionBufferAddDelegate = connectionBufferAddDelegate ?? new GraphNodeDelegate(connectionBuffer.Add);
				GraphNode graphNode2 = path.path[Mathf.Clamp(num + (start ? 1 : (-1)), 0, path.path.Count - 1)];
	...
	public Vector3 GetClampedPoint(Vector3 from, Vector3 to, GraphNode hint)
	{
		...
		if (useGraphRaycasting && hint != null && AstarData.GetGraph(hint) is IRaycastableGraph raycastableGraph && raycastableGraph.Linecast(from, vector, hint, out var hit))
		{
			vector = hit.point;
		}
		return vector;
