...
public static class PathUtilities
{
	...
	public static void GetPointsAroundPoint(Vector3 p, IRaycastableGraph g, List<Vector3> previousPoints, float radius, float clearanceRadius)
	{
		...
		NNInfo nearestForce = ((g as NavGraph) ?? throw new ArgumentException("g is not a NavGraph")).GetNearestForce(p, NNConstraint.Default);
		...
		for (int i = 0; i < previousPoints.Count; i++)
		{
			...
			do
			{
				...
				if (g.Linecast(p, vector2, nearestForce.node, out var hit))
				{
					vector2 = hit.point;
				}
	...
	public static List<Vector3> GetPointsOnNodes(List<GraphNode> nodes, int count, float clearanceRadius = 0f)
	{
		...
		if (nodes[0] is TriangleMeshNode || nodes[0] is GridNode)
		{
			...
			for (int i = 0; i < nodes.Count; i++)
			{
				...
				else if (nodes[i] is GridNode gridNode)
				{
			...
			for (int j = 0; j < count; j++)
			{
				...
				while (!flag)
				{
					...
					Vector3 vector;
					if (nodes[num6] is TriangleMeshNode triangleMeshNode2)
					{
						float num7;
						float num8;
						do
					...
					{
						if (!(nodes[num6] is GridNode gridNode2))
						{
