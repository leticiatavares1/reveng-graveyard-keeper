...
public class RaycastModifierXY : MonoModifier
{
	...
	public bool ValidateLine(GraphNode n1, GraphNode n2, Vector3 v1, Vector3 v2)
	{
		if (useRaycasting)
		{
			...
			{
				if ((bool)Physics2D.CircleCast(v1 + raycastOffset, thickRaycastRadius, v2 - v1, (v2 - v1).magnitude, mask))
				{
					return false;
				}
			}
			else if ((bool)Physics2D.Linecast(v1 + raycastOffset, v2 + raycastOffset, mask))
			{
				return false;
			}
		}
		...
		if (useGraphRaycasting && n1 != null && n2 != null)
		{
			...
			if (graph != null && graph is IRaycastableGraph raycastableGraph && raycastableGraph.Linecast(v1, v2, n1))
			{
				return false;
			}
		}
