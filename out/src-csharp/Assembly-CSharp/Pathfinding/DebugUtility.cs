...
public class DebugUtility : MonoBehaviour
{
	...
	public static void DrawCubes(Vector3[] topVerts, Vector3[] bottomVerts, Color[] vertexColors, float width)
	{
		...
		_ = active.optimizeMeshes;
		GameObject obj = new GameObject("DebugMesh");
		(obj.AddComponent(typeof(MeshRenderer)) as MeshRenderer).material = active.defaultMaterial;
		(obj.AddComponent(typeof(MeshFilter)) as MeshFilter).mesh = mesh;
	}
