		...
		if (!base.error)
		{
			...
			if (logMode == PathLog.Heavy)
			{
				...
				for (int i = 0; i < targetsFound.Length; i++)
				{
					...
					if (nodePaths[i][nodePaths[i].Count - 1] != null)
					{
				...
				debugStringBuilder.AppendLine((base.pathHandler.GetHeap() == null) ? "Null" : (base.pathHandler.GetHeap().numberOfItems - 2).ToString());
			}
		}
