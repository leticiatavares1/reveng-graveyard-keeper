...
{
	private struct GetNearestHelper
	{
		...
		{
			if (!nodeLookup.TryGetValue(p, out var value))
			{
				return;
			}
			while (value != null)
			{
				float sqrMagnitude = ((Vector2)position - (Vector2)value.position).sqrMagnitude;
				if (sqrMagnitude < minDist)
				{
					minDist = sqrMagnitude;
					minNode = value;
				}
				if (constraint == null || (sqrMagnitude < minConstDist && sqrMagnitude < maxDistSqr && constraint.Suitable(value)))
				{
	...
	public override NNInfo GetNearestForce(Vector3 position, NNConstraint constraint)
	{
		...
		float num = (constraint.constrainDistance ? AstarPath.active.maxNearestNodeDistanceSqr : float.PositiveInfinity);
		...
		if (optimizeForSparseGraph)
		{
			...
			val = Math.Max(val, Math.Abs(int2.y));
			val = Math.Max(val, Math.Abs(int2.z));
			GetNearestHelper getNearestHelper = new GetNearestHelper(position, num, constraint, nodeLookup);
			getNearestHelper.Search(@int);
			for (int i = 1; i <= val; i++)
			{
				if (i >= 20)
				{
				...
				if (lookupCellSize.y == 0)
				{
					...
					for (int j = 0; j <= 2 * i; j++)
					{
						getNearestHelper.Search(int3 + new Int3(j, 0, 0));
						getNearestHelper.Search(int3 + new Int3(j, 0, 2 * i));
					}
					for (int k = 1; k < 2 * i; k++)
					{
						getNearestHelper.Search(int3 + new Int3(0, 0, k));
						getNearestHelper.Search(int3 + new Int3(2 * i, 0, k));
					}
				}
				else
				{
					...
					for (int l = 0; l <= 2 * i; l++)
					{
						for (int m = 0; m <= 2 * i; m++)
						{
							getNearestHelper.Search(int4 + new Int3(l, m, 0));
							getNearestHelper.Search(int4 + new Int3(l, m, 2 * i));
						}
					}
					for (int n = 1; n < 2 * i; n++)
					{
						...
						{
							getNearestHelper.Search(int4 + new Int3(0, num4, n));
							getNearestHelper.Search(int4 + new Int3(2 * i, num4, n));
						}
					}
					for (int num5 = 1; num5 < 2 * i; num5++)
					{
						...
						{
							getNearestHelper.Search(int4 + new Int3(num5, 0, num6));
							getNearestHelper.Search(int4 + new Int3(num5, 2 * i, num6));
						}
					}
				}
				graphNode2 = getNearestHelper.minConstNode;
				graphNode = getNearestHelper.minNode;
				num2 = getNearestHelper.minDist;
				num3 = getNearestHelper.minConstDist;
				if (graphNode2 != null)
				{
		...
		else
		{
			for (int num7 = 0; num7 < nodeCount; num7++)
			{
				...
				float sqrMagnitude = ((Vector2)position - (Vector2)pointNode.position).sqrMagnitude;
		...
		if (graphNode2 != null)
		{
			result.constClampedPosition = (Vector2)graphNode2.position;
		}
		else if (graphNode != null)
		{
			result.constrainedNode = graphNode;
			result.constClampedPosition = (Vector2)graphNode.position;
		}
		return result;
	}
	...
	public void RebuildNodeLookup()
	{
		if (optimizeForSparseGraph)
		{
			...
			{
				lookupCellSize.x = Mathf.CeilToInt(1000f * ((limits.x != 0f) ? Mathf.Min(maxDistance, limits.x) : maxDistance));
				lookupCellSize.y = Mathf.CeilToInt(1000f * ((limits.y != 0f) ? Mathf.Min(maxDistance, limits.y) : maxDistance));
				lookupCellSize.z = Mathf.CeilToInt(1000f * ((limits.z != 0f) ? Mathf.Min(maxDistance, limits.z) : maxDistance));
			}
			if (optimizeFor2D)
			{
				lookupCellSize.y = 0;
	...
	private void AddToLookup(PointNode node)
	{
		if (nodeLookup != null)
		{
			...
			if (nodeLookup.TryGetValue(key, out var value))
			{
	...
	public override void ScanInternal(OnScanStatus statusCallback)
	{
		if (root == null)
		{
			GameObject[] array = ((searchTag != null) ? GameObject.FindGameObjectsWithTag(searchTag) : null);
			if (array == null)
			{
		...
		for (int m = 0; m < nodes.Length; m++)
		{
			...
			if (optimizeForSparseGraph)
			{
				...
				int num2 = ((lookupCellSize.y == 0) ? 9 : ThreeDNeighbours.Length);
				for (int n = 0; n < num2; n++)
				{
					...
					while (value != null)
					{
						if (IsValidConnection(pointNode, value, out var dist))
						{
							list.Add(value);
			...
			else
			{
				for (int num3 = 0; num3 < nodes.Length; num3++)
				{
					if (m != num3)
					{
						...
						if (IsValidConnection(pointNode, pointNode2, out var dist2))
						{
							list.Add(pointNode2);
			...
			pointNode.connections = connections;
