...
[Serializable]
[AddComponentMenu("Pathfinding/Modifiers/Funnel XY")]
[HelpURL("http://arongranberg.com/astar/docs/class_pathfinding_1_1_funnel_modifier.php")]
public class FunnelModifierXY : MonoModifier
{
	...
	public override void Apply(Path p)
	{
		...
		{
			if (!path[i].GetPortal(path[i + 1], list2, list3, backwards: false))
			{
	...
	public static bool RunFunnel(List<Vector3> left, List<Vector3> right, List<Vector3> funnelPath)
	{
		...
		while (VectorMathXY.IsColinearXY(left[0], left[1], right[1]) || VectorMathXY.RightOrColinearXY(left[1], right[1], vector) == VectorMathXY.RightOrColinearXY(left[1], right[1], left[0]))
		{
			left.RemoveAt(1);
			right.RemoveAt(1);
			if (left.Count <= 3)
		...
		if (!VectorMathXY.IsClockwiseXY(left[0], left[1], right[1]) && !VectorMathXY.IsColinearXY(left[0], left[1], right[1]))
		{
		...
		for (int i = 2; i < left.Count; i++)
		{
			...
			if (VectorMathXY.SignedTriangleAreaTimes2XY(vector2, vector4, vector6) >= 0f)
			{
				if (!(vector2 == vector4) && !(VectorMathXY.SignedTriangleAreaTimes2XY(vector2, vector3, vector6) <= 0f))
				{
			...
			if (VectorMathXY.SignedTriangleAreaTimes2XY(vector2, vector3, vector5) <= 0f)
			{
				if (vector2 == vector3 || VectorMathXY.SignedTriangleAreaTimes2XY(vector2, vector4, vector5) >= 0f)
				{
					vector3 = vector5;
					num2 = i;
					continue;
