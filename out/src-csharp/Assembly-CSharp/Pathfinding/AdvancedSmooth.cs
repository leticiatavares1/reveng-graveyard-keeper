...
public class AdvancedSmooth : MonoModifier
{
	...
	public class MaxTurn : TurnConstructor
	{
		...
		public override void TangentToTangent(List<Turn> turnList)
		{
			...
			deltaRightLeft = (flag ? 0.0 : (Math.PI / 2.0 - Math.Asin((double)(TurnConstructor.turningRadius * 2f) / num)));
			deltaLeftRight = (flag2 ? 0.0 : (Math.PI / 2.0 - Math.Asin((double)(TurnConstructor.turningRadius * 2f) / num2)));
		...
		public override void PointToTangent(List<Turn> turnList)
		{
			...
			double num = (flag ? 0.0 : Atan2(TurnConstructor.prev - rightCircleCenter));
			double num2 = (flag ? 0.0 : (Math.PI / 2.0 - Math.Asin(TurnConstructor.turningRadius / (TurnConstructor.prev - rightCircleCenter).magnitude)));
			gammaRight = num + num2;
			double num3 = (flag ? 0.0 : ClockwiseAngle(gammaRight, vaRight));
			double num4 = (flag2 ? 0.0 : Atan2(TurnConstructor.prev - leftCircleCenter));
			double num5 = (flag2 ? 0.0 : (Math.PI / 2.0 - Math.Asin(TurnConstructor.turningRadius / (TurnConstructor.prev - leftCircleCenter).magnitude)));
			gammaLeft = num4 - num5;
			double num6 = (flag2 ? 0.0 : CounterClockwiseAngle(gammaLeft, vaLeft));
			if (!flag)
			{
	...
	public class ConstantTurn : TurnConstructor
	{
		...
		public override void TangentToTangent(List<Turn> turnList)
		{
			...
			circleCenter = VectorMath.LineDirIntersectionPointXZ(TurnConstructor.prev, dir, start, vector, out var intersects);
			if (intersects)
			{
				...
				double angle = (clockwise ? ClockwiseAngle(gamma1, gamma2) : CounterClockwiseAngle(gamma1, gamma2));
