...
public class AstarDebugger : MonoBehaviour
{
	...
	public void Update()
	{
		...
		peakAlloc = ((!flag) ? allocMem : peakAlloc);
		...
		{
			fpsDrops[Time.frameCount % fpsDrops.Length] = ((Time.deltaTime != 0f) ? (1f / Time.deltaTime) : float.PositiveInfinity);
			int num3 = Time.frameCount % graph.Length;
			graph[num3].fps = ((Time.deltaTime < Mathf.Epsilon) ? 0f : (1f / Time.deltaTime));
