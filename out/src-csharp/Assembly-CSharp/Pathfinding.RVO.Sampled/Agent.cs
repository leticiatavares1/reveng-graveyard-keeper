...
public class Agent : IAgent
{
	...
	public void CalculateNeighbours()
	{
		...
		if (!locked)
		{
			...
			obstacles.Clear();
			obstacleDists.Clear();
			_ = obstacleTimeHorizon * maxSpeed + radius;
		}
	}
	...
	internal void CalculateVelocity(Simulator.WorkerContext context)
	{
		...
		float num3 = ((simulator.algorithm == Simulator.SamplingAlgorithm.GradientDescent) ? 1f : 5f);
		for (int i = 0; i < simulator.obstacles.Count; i++)
		{
			...
			do
			{
				...
				float num5 = Vector2.Dot(obstacleVertex2.dir, vector - new Vector2(obstacleVertex2.position.x, obstacleVertex2.position.z));
				bool flag = num5 <= wallThickness * 0.05f || num5 >= (new Vector2(obstacleVertex2.position.x, obstacleVertex2.position.z) - new Vector2(obstacleVertex2.next.position.x, obstacleVertex2.next.position.z)).magnitude - wallThickness * 0.05f;
				if (Mathf.Abs(num4) < neighbourDist)
				{
					if (num4 <= 0f && !flag && num4 > 0f - wallThickness)
					{
		...
		for (int j = 0; j < neighbours.Count; j++)
		{
			...
			float num6 = Math.Min(position.y + height, agent.position.y + agent.height);
			float num7 = Math.Max(position.y, agent.position.y);
			if (!(num6 - num7 < 0f))
			{
				Vector2 vector3 = new Vector2(agent.Velocity.x, agent.velocity.z);
				float num8 = radius + agent.radius;
				...
				vos[num] = new VO(vector4, vector5, num8, sideChooser, num2, 1f);
				num++;
				if (DebugDraw)
				{
					DrawVO(vector + vector4 * num2 + vector5, num8 * num2, vector + vector5);
				}
			}
		}
			...
			if (DebugDraw)
			{
				for (int k = 0; k < 40; k++)
				{
					for (int l = 0; l < 40; l++)
					{
						...
						for (int m = 0; m < num; m++)
						{
							zero2 += vos[m].Sample(vector6 - vector, out var num10);
							if (num10 > num9)
							{
								num9 = num10;
							}
						...
						if (vector7.magnitude * DesiredVelocityWeight > num9)
						{
							num9 = vector7.magnitude * DesiredVelocityWeight;
						}
						if (num9 > 0f)
						{
							zero2 /= num9;
		...
		else
		{
			...
			float num12 = Mathf.Max(radius, Mathf.Max(vector11.magnitude, Velocity.magnitude));
			samplePos[num11] = vector11;
			sampleSize[num11] = num12 * 0.3f;
			num11++;
			samplePos[num11] = vector2;
			sampleSize[num11] = num12 * 0.3f;
			num11++;
			...
			for (int n = 0; n < 8; n++)
			{
				samplePos[num11] = vector13 * Mathf.Sin((float)n * (float)Math.PI * 2f / 8f) + vector12 * (1f + Mathf.Cos((float)n * (float)Math.PI * 2f / 8f));
				sampleSize[num11] = (1f - Mathf.Abs((float)n - 4f) / 8f) * num12 * 0.5f;
				num11++;
			}
			vector12 *= 0.6f;
			vector13 *= 0.6f;
			...
			{
				samplePos[num11] = vector13 * Mathf.Cos(((float)num13 + 0.5f) * (float)Math.PI * 2f / 6f) + vector12 * (1.6666666f + Mathf.Sin(((float)num13 + 0.5f) * (float)Math.PI * 2f / 6f));
				sampleSize[num11] = num12 * 0.3f;
				num11++;
			}
			for (int num14 = 0; num14 < 6; num14++)
			{
				samplePos[num11] = vector2 + new Vector2(num12 * 0.2f * Mathf.Cos(((float)num14 + 0.5f) * (float)Math.PI * 2f / 6f), num12 * 0.2f * Mathf.Sin(((float)num14 + 0.5f) * (float)Math.PI * 2f / 6f));
				sampleSize[num11] = num12 * 0.2f * 2f;
				num11++;
			}
			samplePos[num11] = vector2 * 0.5f;
			sampleSize[num11] = num12 * 0.4f;
			num11++;
			...
			{
				bestScores[num15] = float.PositiveInfinity;
			}
			...
			for (int num17 = 0; num17 < 3; num17++)
			{
				for (int num18 = 0; num18 < num11; num18++)
				{
					...
					{
						num19 = Math.Max(num19, vos[num20].ScalarSample(samplePos[num18]));
					}
					float magnitude = (samplePos[num18] - vector11).magnitude;
					float num21 = num19 + magnitude * DesiredVelocityWeight;
					num19 += magnitude * 0.001f;
					if (DebugDraw)
					{
						DrawCross(vector + samplePos[num18], Rainbow(Mathf.Log(num19 + 1f) * 5f), sampleSize[num18] * 0.5f);
					}
					if (num21 < bestScores[0])
					{
						for (int num22 = 0; num22 < 3; num22++)
						{
							if (num21 >= bestScores[num22 + 1])
							{
								bestScores[num22] = num21;
								bestSizes[num22] = sampleSize[num18];
								bestPos[num22] = samplePos[num18];
								break;
							}
						}
					}
				...
				{
					Vector2 vector15 = bestPos[num23];
					float num24 = bestSizes[num23];
					bestScores[num23] = float.PositiveInfinity;
					float num25 = num24 * 0.6f * 0.5f;
					samplePos[num11] = vector15 + new Vector2(num25, num25);
					samplePos[num11 + 1] = vector15 + new Vector2(0f - num25, num25);
					samplePos[num11 + 2] = vector15 + new Vector2(0f - num25, 0f - num25);
					samplePos[num11 + 3] = vector15 + new Vector2(num25, 0f - num25);
					sampleSize[num11 + 3] = (sampleSize[num11 + 2] = (sampleSize[num11 + 1] = (sampleSize[num11] = num24 * (num24 * 0.6f))));
					num11 += 4;
				}
			}
			zero = vector14;
