...
public class AstarPath : MonoBehaviour
{
		...
		if (showNavGraphs && !manualDebugFloorRoof)
		{
			...
			for (int i = 0; i < graphs.Length; i++)
			{
				...
				graphs[i].GetNodes(delegate(GraphNode node)
				{
					...
					{
						PathNode pathNode = ((debugPathData != null) ? debugPathData.GetPathNode(node) : null);
	...
	private void QueueGraphUpdatesInternal()
	{
		...
		while (graphUpdateQueue.Count > 0)
		{
			...
			{
				GUOSingle gUOSingle = default(GUOSingle);
				gUOSingle.order = GraphUpdateOrder.FloodFill;
				gUOSingle.obj = graphUpdateObject;
				GUOSingle item2 = gUOSingle;
	...
	private bool ProcessGraphUpdates(bool force)
	{
		...
		while (graphUpdateQueueRegular.Count > 0)
		{
			...
			{
				FloodFill(item.obj.only_specific_graph);
				continue;
			}
	...
	private void Awake()
	{
		...
		if (scanOnStartup && can_scan_on_startup && (!astarData.cacheStartup || astarData.file_cachedStartup == null))
		{
			Scan();
			Debug.Log("A* scan on awake");
		}
	}
	...
	public void FloodFill(int only_specific_graph = -1)
	{
		...
		for (int j = 0; j < graphs.Length; j++)
		{
			if (only_specific_graph != -1 && j != only_specific_graph)
			{
				continue;
			}
			...
			{
				if (node != null && node.Walkable && node.Area == 0)
				{
					area++;
					uint num = area;
					if (area > 131071)
	...
	{
		Debug.Log("A* scan, graph = " + only_specific_graph);
		ScanLoop(null, only_specific_graph);
	}
	...
	public void ScanLoop(OnScanStatus statusCallback, int only_specific_graph = -1)
	{
		...
		for (int j = 0; j < graphs.Length; j++)
		{
			if ((only_specific_graph == -1 || j == only_specific_graph) && graphs[j] != null)
			{
		...
		while (true)
		{
			...
			if (!p.IsDone())
			{
				...
				while (!p.IsDone())
				{
					...
					totalTicks2 += DateTime.UtcNow.Ticks - ticks;
					yield return null;
					ticks = DateTime.UtcNow.Ticks;
					if (astar.pathQueue.IsTerminating)
					{
