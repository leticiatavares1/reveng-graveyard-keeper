...
public class AstarSerializer
{
	...
	private NavGraph DeserializeGraph(int zipIndex, int graphIndex)
	{
		...
		else
		{
			...
			GraphSerializationContext ctx = new GraphSerializationContext(GetBinaryReader(GetEntry(name2)), null, navGraph.graphIndex, meta);
	...
	private bool DeserializeExtraInfo(NavGraph graph)
	{
		...
		GraphSerializationContext ctx = new GraphSerializationContext(GetBinaryReader(entry), null, graph.graphIndex, meta);
	...
	private void DeserializeNodeReferences(NavGraph graph, GraphNode[] int2Node)
	{
		...
		BinaryReader binaryReader = GetBinaryReader(GetEntry("graph" + num + "_references.binary") ?? throw new Exception("Node references for graph " + num + " not found in the data. Was this loaded from an older version of the A* Pathfinding Project?"));
	...
	private void DeserializeNodeLinks(GraphNode[] int2Node)
	{
		...
		if (entry != null)
		{
			NodeLink2.DeserializeReferences(new GraphSerializationContext(GetBinaryReader(entry), int2Node, 0u, meta));
		}
	}
