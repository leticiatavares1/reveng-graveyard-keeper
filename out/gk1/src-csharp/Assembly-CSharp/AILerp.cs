...
public class AILerp : MonoBehaviour
{
	...
	{
		bool num = path != null && path.vectorPath != null && path.vectorPath.Count > 1;
		bool flag = false;
		if (num)
		{
			flag = currentWaypointIndex == path.vectorPath.Count - 1 && distanceAlongSegment >= (path.vectorPath[path.vectorPath.Count - 1] - path.vectorPath[path.vectorPath.Count - 2]).magnitude;
		}
		if (num && !flag)
		{
			...
			float num2 = vector.magnitude * Mathf.Clamp01(1f - distanceAlongSegment);
			for (int i = currentWaypointIndex; i < vectorPath.Count - 1; i++)
			{
				num2 += (vectorPath[i + 1] - vectorPath[i]).magnitude;
			}
			previousMovementOrigin = GetFeetPosition();
			previousMovementDirection = vector.normalized * num2;
	...
	protected virtual Vector3 CalculateNextPosition(out Vector3 direction)
	{
		...
		Vector3 vector3 = vector * Mathf.Clamp01((num > 0f) ? (distanceAlongSegment / num) : 1f) + vectorPath[currentWaypointIndex - 1];
		direction = vector;
		if (interpolatePathSwitches)
		{
			return Vector3.Lerp(previousMovementOrigin + Vector3.ClampMagnitude(previousMovementDirection, speed * (Time.time - previousMovementStartTime)), vector3, switchPathInterpolationSpeed * (Time.time - previousMovementStartTime));
		}
		return vector3;
	}
}
