	...
	public void SimplifyPath(IRaycastableGraph graph, List<GraphNode> nodes, int start, int end, List<GraphNode> result, Vector3 startPoint, Vector3 endPoint)
	{
		...
		while (true)
		{
			...
			while (num3 > num4 + 1)
			{
				...
				Vector3 end2 = ((num5 == end) ? endPoint : ((Vector3)nodes[num5].position));
			...
			Vector3 end3 = ((num4 == end) ? endPoint : ((Vector3)nodes[num4].position));
		...
		else
		{
			...
			if (!flag)
			{
				...
				for (int count2 = nodes.Count; k < count2; k++)
				{
					...
					float sqrMagnitude = (nodes[k].ClosestPointOnNode(position) - position).sqrMagnitude;
				...
				{
					if ((containingIndex <= 0 || node != nodes[containingIndex - 1]) && (containingIndex >= nodes.Count - 1 || node != nodes[containingIndex + 1]) && node is TriangleMeshNode triangleMeshNode)
					{
	...
	private void FindWalls(int nodeIndex, List<Vector3> wallBuffer, Vector3 position, float range)
	{
		...
		while (!flag || !flag2)
		{
			if (!(num < 0 && flag) && !(num > 0 && flag2))
			{
				...
				{
					TriangleMeshNode triangleMeshNode = ((nodeIndex + num - 1 < 0) ? null : nodes[nodeIndex + num - 1]);
					TriangleMeshNode triangleMeshNode2 = nodes[nodeIndex + num];
					TriangleMeshNode triangleMeshNode3 = ((nodeIndex + num + 1 >= nodes.Count) ? null : nodes[nodeIndex + num + 1]);
