...
public class NavmeshCut : MonoBehaviour
{
	...
	private static bool Intersects(Bounds b1, Bounds b2)
	{
		...
		Vector3 min2 = b2.min;
		Vector3 max2 = b2.max;
		if (min.x <= max2.x && max.x >= min2.x && min.y <= max2.y && max.y >= min2.y && min.z <= max2.z)
		{
			return max.z >= min2.z;
		}
		return false;
	...
	public void GetContour(List<List<IntPoint>> buffer)
	{
		...
		case MeshType.Rectangle:
		{
			...
			flag ^= (rectangleSize.x < 0f) ^ (rectangleSize.y < 0f);
			if (useRotation)
			{
				list.Add(V3ToIntPoint(matrix.MultiplyPoint3x4(center + new Vector3(0f - rectangleSize.x, 0f, 0f - rectangleSize.y) * 0.5f)));
				list.Add(V3ToIntPoint(matrix.MultiplyPoint3x4(center + new Vector3(rectangleSize.x, 0f, 0f - rectangleSize.y) * 0.5f)));
				list.Add(V3ToIntPoint(matrix.MultiplyPoint3x4(center + new Vector3(rectangleSize.x, 0f, rectangleSize.y) * 0.5f)));
				list.Add(V3ToIntPoint(matrix.MultiplyPoint3x4(center + new Vector3(0f - rectangleSize.x, 0f, rectangleSize.y) * 0.5f)));
			}
			else
			{
				position += center;
		...
		case MeshType.Circle:
		{
			...
			flag ^= circleRadius < 0f;
			if (useRotation)
			{
				for (int l = 0; l < circleResolution; l++)
				{
					list.Add(V3ToIntPoint(matrix.MultiplyPoint3x4(center + new Vector3(Mathf.Cos((float)(l * 2) * (float)Math.PI / (float)circleResolution), 0f, Mathf.Sin((float)(l * 2) * (float)Math.PI / (float)circleResolution)) * circleRadius)));
				}
			}
			else
			{
		...
		case MeshType.CustomMesh:
		{
			...
			for (int i = 0; i < contours.Length; i++)
			{
				...
				if (useRotation)
				{
					...
					{
						list.Add(V3ToIntPoint(matrix.MultiplyPoint3x4(center + array[j] * meshScale)));
					}
				}
				else
				{
