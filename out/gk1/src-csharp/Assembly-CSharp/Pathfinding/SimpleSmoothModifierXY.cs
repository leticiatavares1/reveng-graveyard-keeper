	...
	public List<Vector3> SmoothOffsetSimple(List<Vector3> path)
	{
		...
		for (int k = 0; k < iterations; k++)
		{
			...
			for (int l = 0; l < num2 - 1; l++)
			{
				...
				bool flag3 = false;
				bool flag4 = false;
				if (l != 0 && !VectorMathXY.IsColinearXY(vector, vector2, list2[l - 1]))
				{
					flag3 = true;
					flag = VectorMathXY.RightOrColinearXY(vector, vector2, list2[l - 1]);
				}
				if (l < num2 - 1 && !VectorMathXY.IsColinearXY(vector, vector2, list2[l + 2]))
				{
					flag4 = true;
					flag2 = VectorMathXY.RightOrColinearXY(vector, vector2, list2[l + 2]);
				}
				if (flag3)
				{
		...
		if (strength > 0f)
		{
			for (int m = 0; m < iterations; m++)
			{
				...
				for (int n = 1; n < list.Count - 1; n++)
				{
					...
					list[n] = Vector3.Lerp(vector2, (vector + list[n + 1]) / 2f, strength);
					vector = vector2;
				}
			}
		}
