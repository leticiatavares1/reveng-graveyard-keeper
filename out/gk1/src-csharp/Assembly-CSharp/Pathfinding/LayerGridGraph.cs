...
public class LayerGridGraph : GridGraph, IUpdatableGraph
{
	...
	public new void UpdateArea(GraphUpdateObject o)
	{
		...
		GridGraph.GetBoundsMinMax(o.bounds, inverseMatrix, out var min, out var max);
				...
				for (int num2 = intRect4.ymin; num2 <= intRect4.ymax; num2++)
				{
					for (int num3 = 0; num3 < layerCount; num3++)
					{
						...
						if (levelGridNode != null)
						{
							...
							CalculateConnections(array, levelGridNode, n, num2, num3);
						}
					}
				}
			}
			...
			for (int num9 = intRect4.xmin; num9 <= intRect4.xmax; num9++)
			{
				for (int num10 = intRect4.ymin; num10 <= intRect4.ymax; num10++)
				{
					for (int num11 = 0; num11 < layerCount; num11++)
					{
						...
						{
							GraphNode[] array = nodes;
							CalculateConnections(array, levelGridNode3, num9, num10, num11);
						}
					}
				}
			}
			...
			for (int num17 = a2.xmin; num17 <= a2.xmax; num17++)
			{
				for (int num18 = a2.ymin; num18 <= a2.ymax; num18++)
				{
					for (int num19 = 0; num19 < layerCount; num19++)
					{
						...
						{
							GraphNode[] array = nodes;
							CalculateConnections(array, levelGridNode5, num17, num18, num19);
						}
					}
				}
			}
			...
			for (int num25 = a2.xmin; num25 <= a2.xmax; num25++)
			{
				for (int num26 = a2.ymin; num26 <= a2.ymax; num26++)
				{
					for (int num27 = 0; num27 < layerCount; num27++)
					{
						...
						{
							GraphNode[] array = nodes;
							CalculateConnections(array, levelGridNode7, num25, num26, num27);
						}
					}
				}
			}
	...
	public override void ScanInternal(OnScanStatus statusCallback)
	{
		...
		for (int m = 0; m < depth; m++)
		{
			...
			{
				LinkedLevelNode linkedLevelNode4 = array[m * width + n].first;
				int num2 = 0;
				do
				{
					num2++;
				...
				layerCount = ((num2 > layerCount) ? num2 : layerCount);
			}
		}
		if (layerCount > 255)
		{
		...
		for (int num6 = 0; num6 < depth; num6++)
		{
			...
			{
				LinkedLevelCell linkedLevelCell2 = array[num6 * width + num7];
				LinkedLevelNode linkedLevelNode5 = linkedLevelCell2.first;
				linkedLevelCell2.index = num4;
				int num8 = 0;
				int num9 = 0;
				do
				{
		...
		for (int num11 = 0; num11 < depth; num11++)
		{
			for (int num12 = 0; num12 < width; num12++)
			{
				for (int num13 = 0; num13 < layerCount; num13++)
				{
					...
					GraphNode[] array3 = nodes;
					CalculateConnections(array3, node, num12, num11, num13);
				}
			}
		}
			...
			for (int n = 0; n < layerCount; n++)
			{
				for (int num = zmin; num < zmax; num++)
				{
					for (int num2 = xmin; num2 < xmax; num2++)
					{
						...
						{
							GraphNode[] array = nodes;
							CalculateConnections(array, levelGridNode2, num2, num, n);
						}
					}
				}
			}
		...
		for (int i = 0; i < 4; i++)
		{
			...
			for (int j = 0; j < layerCount; j++)
			{
				...
				if (graphNode != null && graphNode.Walkable)
				{
					...
					if (Mathf.Min((float)graphNode.position.y * 0.001f + num5, (float)levelGridNode.position.y * 0.001f + num) - num6 >= characterHeight && (float)Mathf.Abs(graphNode.position.y - levelGridNode.position.y) * 0.001f <= maxClimb)
					{
						value = j;
					}
				}
	...
	public override NNInfo GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
	{
		...
		return new NNInfo(GetNearestNode(position, x, z, null));
	}
