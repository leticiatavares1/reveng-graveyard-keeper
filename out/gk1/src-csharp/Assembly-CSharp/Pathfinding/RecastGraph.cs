...
public class RecastGraph : NavGraph, INavmesh, IRaycastableGraph, IUpdatableGraph, INavmeshHolder
{
		...
		if (tiles != null)
		{
			...
			for (int i = 0; i < tiles.Length; i++)
			{
				...
				if (navmeshTile != null)
				{
					...
					{
						navmeshTile.nodes[k].UpdatePositionFromVertices();
					}
					BBTree bbTree = navmeshTile.bbTree;
					MeshNode[] nodes = navmeshTile.nodes;
	...
	{
		CollectMeshes(out var extraMeshes, new Bounds(Vector3.zero, new Vector3(float.PositiveInfinity, float.PositiveInfinity, float.PositiveInfinity)));
	...
	public IntRect GetTouchingTiles(Bounds b)
	{
		...
		return IntRect.Intersection(new IntRect(Mathf.FloorToInt(b.min.x / ((float)tileSizeX * cellSize)), Mathf.FloorToInt(b.min.z / ((float)tileSizeZ * cellSize)), Mathf.FloorToInt(b.max.x / ((float)tileSizeX * cellSize)), Mathf.FloorToInt(b.max.z / ((float)tileSizeZ * cellSize))), new IntRect(0, 0, tileXCount - 1, tileZCount - 1));
	}

	public IntRect GetTouchingTilesRound(Bounds b)
	{
		b.center -= forcedBounds.min;
		return IntRect.Intersection(new IntRect(Mathf.RoundToInt(b.min.x / ((float)tileSizeX * cellSize)), Mathf.RoundToInt(b.min.z / ((float)tileSizeZ * cellSize)), Mathf.RoundToInt(b.max.x / ((float)tileSizeX * cellSize)) - 1, Mathf.RoundToInt(b.max.z / ((float)tileSizeZ * cellSize)) - 1), new IntRect(0, 0, tileXCount - 1, tileZCount - 1));
	}
	...
	void IUpdatableGraph.UpdateAreaInit(GraphUpdateObject o)
	{
		...
		int num = Mathf.CeilToInt(characterRadius / cellSize) + 3;
		tileBounds.Expand(new Vector3(num, 0f, num) * cellSize * 2f);
		CollectMeshes(out var extraMeshes, tileBounds);
	...
	void IUpdatableGraph.UpdateArea(GraphUpdateObject guo)
	{
		...
		for (int k = touchingTiles.ymin; k <= touchingTiles.ymax; k++)
		{
			...
			{
				RemoveConnectionsFromTile(tiles[l + k * tileXCount]);
			}
		}
		...
		for (int num2 = touchingTiles.ymin; num2 <= touchingTiles.ymax; num2++)
		{
			for (int num3 = touchingTiles.xmin; num3 <= touchingTiles.xmax; num3++)
			{
				GraphNode[] nodes = tiles[num3 + num2 * tileXCount].nodes;
				GraphNode[] array = nodes;
				for (int num4 = 0; num4 < array.Length; num4++)
				{
					array[num4].GraphIndex = num;
				}
			}
		}
		...
		for (int num5 = touchingTiles.ymin; num5 <= touchingTiles.ymax; num5++)
		{
			for (int num6 = touchingTiles.xmin; num6 <= touchingTiles.xmax; num6++)
			{
				if (touchingTiles.Contains(num6 + 1, num5))
				{
					ConnectTiles(tiles[num6 + num5 * tileXCount], tiles[num6 + 1 + num5 * tileXCount]);
				}
				if (touchingTiles.Contains(num6, num5 + 1))
				{
					ConnectTiles(tiles[num6 + num5 * tileXCount], tiles[num6 + (num5 + 1) * tileXCount]);
				}
			}
		}
	}
	...
	public void RemoveConnectionsFromTo(NavmeshTile a, NavmeshTile b)
	{
		...
		for (int i = 0; i < a.nodes.Length; i++)
		{
			...
			{
				if (triangleMeshNode.connections[j] is TriangleMeshNode triangleMeshNode2 && ((triangleMeshNode2.GetVertexIndex(0) >> 12) & 0x7FFFF) == num)
				{
	...
	protected void ScanAllTiles(OnScanStatus statusCallback)
	{
		...
		CollectMeshes(out var extraMeshes, forcedBounds);
	...
	private void CreateNodeConnections(TriangleMeshNode[] nodes)
	{
		...
		foreach (TriangleMeshNode triangleMeshNode2 in nodes)
		{
			...
			for (int l = 0; l < vertexCount2; l++)
			{
				...
				if (!dictionary.TryGetValue(new Int2(vertexIndex2, vertexIndex), out var value))
				{
					continue;
				}
	...
	public void ReplaceTile(int x, int z, int w, int d, Int3[] verts, int[] tris, bool worldSpace)
	{
		...
		num2 <<= 12;
		if (navmeshTile3.verts.Length > 4095)
		{
			UnityEngine.Debug.LogError("Too many vertices in the tile (" + navmeshTile3.verts.Length + " > " + 4095 + ")\nYou can enable ASTAR_RECAST_LARGER_TILES under the 'Optimizations' tab in the A* Inspector to raise this limit.");
		...
		BBTree bbTree = navmeshTile3.bbTree;
		MeshNode[] nodes = array;
		bbTree.RebuildFrom(nodes);
	...
	private void CollectTerrainMeshes(Bounds bounds, bool rasterizeTrees, List<ExtraMesh> extraMeshes)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			int num = (terrainData.heightmapResolution + terrainSampleSize - 1) / terrainSampleSize + 1;
			int num2 = (terrainData.heightmapResolution + terrainSampleSize - 1) / terrainSampleSize + 1;
			...
			for (int j = 0; j < num2; j++)
			{
				...
				for (int k = 0; k < num; k++)
				{
					int num5 = Math.Min(num4, heightmapResolution - 1);
					int num6 = Math.Min(num3, heightmapResolution2 - 1);
		...
		if (col is SphereCollider || col is CapsuleCollider)
		{
			...
			if (capsuleCache == null)
			{
				...
				for (int j = 0; j < num3; j++)
				{
					for (int k = 0; k < num4; k++)
					{
						array[k + j * num4] = new Vector3(Mathf.Cos((float)k * (float)Math.PI * 2f / (float)num4) * Mathf.Sin((float)j * (float)Math.PI / (float)(num3 - 1)), Mathf.Cos((float)j * (float)Math.PI / (float)(num3 - 1)) + ((j < num3 / 2) ? num2 : (0f - num2)), Mathf.Sin((float)k * (float)Math.PI * 2f / (float)num4) * Mathf.Sin((float)j * (float)Math.PI / (float)(num3 - 1)));
					}
				}
				...
				for (int l = 1; l < num3; l++)
				{
					...
					while (num6 < num4)
					{
						list.Add(l * num4 + num6);
						list.Add(l * num4 + num7);
						list.Add((l - 1) * num4 + num6);
	...
	public override void OnDrawGizmos(bool drawNodes)
	{
		...
		GraphNodeDelegateCancelable del = delegate(GraphNode _node)
		{
			...
			if (AstarPath.active.showSearchTree && debugData != null)
			{
				...
				if (flag && showNodeConnections && debugData.GetPathNode(triangleMeshNode).parent != null)
				{
				...
				if (showMeshOutline)
				{
					Gizmos.color = (triangleMeshNode.Walkable ? NodeColor(triangleMeshNode, debugData) : AstarColor.UnwalkableNode);
					if (!flag)
					{
