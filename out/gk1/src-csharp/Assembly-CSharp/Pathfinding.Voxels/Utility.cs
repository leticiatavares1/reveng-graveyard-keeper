...
public class Utility
{
	...
	public static int ClipPoly(float[] vIn, int n, float[] vOut, float pnx, float pnz, float pd)
	{
		...
		for (; j < n; j++)
		{
			bool num3 = array[num2] >= 0f;
			bool flag = array[j] >= 0f;
			if (num3 != flag)
			{
				float num4 = array[num2] / (array[num2] - array[j]);
				vOut[num * 3] = vIn[num2 * 3] + (vIn[j * 3] - vIn[num2 * 3]) * num4;
				vOut[num * 3 + 1] = vIn[num2 * 3 + 1] + (vIn[j * 3 + 1] - vIn[num2 * 3 + 1]) * num4;
				vOut[num * 3 + 2] = vIn[num2 * 3 + 2] + (vIn[j * 3 + 2] - vIn[num2 * 3 + 2]) * num4;
				num++;
			}
			if (flag)
			{
	...
	public static int ClipPolygon(float[] vIn, int n, float[] vOut, float multi, float offset, int axis)
	{
		...
		for (; j < n; j++)
		{
			...
			if (num3 != flag)
			{
				...
				int num5 = j * 3;
				int num6 = num2 * 3;
				float num7 = array[num2] / (array[num2] - array[j]);
				vOut[num4] = vIn[num6] + (vIn[num5] - vIn[num6]) * num7;
				vOut[num4 + 1] = vIn[num6 + 1] + (vIn[num5 + 1] - vIn[num6 + 1]) * num7;
				vOut[num4 + 2] = vIn[num6 + 2] + (vIn[num5 + 2] - vIn[num6 + 2]) * num7;
				num++;
			}
			if (flag)
			{
				...
				int num9 = j * 3;
				vOut[num8] = vIn[num9];
				vOut[num8 + 1] = vIn[num9 + 1];
				vOut[num8 + 2] = vIn[num9 + 2];
				num++;
			}
			num2 = j;
		}
