...
public class Voxelize
{
	...
	{
		int[] array = ClaimIntArr((ca.nverts + cb.nverts + 2) * 4, zero: false);
		int num = 0;
	...
	public void SimplifyContour(List<int> verts, List<int> simplified, float maxError, int maxEdgeLenght, int buildFlags)
	{
		...
		while (num12 < simplified.Count / 4)
		{
			...
			int num14 = simplified[num12 * 4];
			int num15 = simplified[num12 * 4 + 2];
			int num16 = simplified[num12 * 4 + 3];
			int num17 = simplified[num13 * 4];
			int num18 = simplified[num13 * 4 + 2];
			int num19 = simplified[num13 * 4 + 3];
			float num20 = 0f;
			int num21 = -1;
			int num22;
			int num23;
			int num24;
			if (num17 > num14 || (num17 == num14 && num18 > num15))
			{
				num22 = 1;
				num23 = (num16 + num22) % num11;
				num24 = num19;
			}
			else
			{
				num22 = num11 - 1;
				num23 = (num19 + num22) % num11;
				num24 = num16;
			}
			if ((verts[num23 * 4 + 3] & 0xFFFF) == 0 || (verts[num23 * 4 + 3] & 0x20000) == 131072)
			{
				while (num23 != num24)
				{
					float num25 = VectorMath.SqrDistancePointSegmentApproximate(verts[num23 * 4], verts[num23 * 4 + 2] / voxelArea.width, num14, num15 / voxelArea.width, num17, num18 / voxelArea.width);
					if (num25 > num20)
					{
						num20 = num25;
						num21 = num23;
					}
					num23 = (num23 + num22) % num11;
				}
			}
			if (num21 != -1 && num20 > maxError)
			{
				simplified.Add(0);
				simplified.Add(0);
				simplified.Add(0);
				simplified.Add(0);
				for (int num26 = simplified.Count / 4 - 1; num26 > num12; num26--)
				{
				...
				simplified[(num12 + 1) * 4] = verts[num21 * 4];
				simplified[(num12 + 1) * 4 + 1] = verts[num21 * 4 + 1];
				simplified[(num12 + 1) * 4 + 2] = verts[num21 * 4 + 2];
				simplified[(num12 + 1) * 4 + 3] = num21;
			}
			else
			{
				num12++;
		...
		if (num27 > 0f && ((uint)buildFlags & 3u) != 0)
		{
			int num28 = 0;
			while (num28 < simplified.Count / 4 && simplified.Count / 4 <= 200)
			{
				...
				if (flag3)
				{
					...
					if ((float)(num38 * num38 + num39 * num39) > num27 * num27)
					{
						...
						{
							int num40 = ((num35 < num32) ? (num35 + num11 - num32) : (num35 - num32));
							num36 = (num32 + num40 / 2) % num11;
						}
						else
						{
							int num41 = ((num35 < num32) ? (num35 + num11 - num32) : (num35 - num32));
							num36 = (num32 + (num41 + 1) / 2) % num11;
						}
					}
				}
				if (num36 != -1)
				{
					...
					for (int num42 = simplified.Count / 4 - 1; num42 > num28; num42--)
					{
						simplified[num42 * 4] = simplified[(num42 - 1) * 4];
						simplified[num42 * 4 + 1] = simplified[(num42 - 1) * 4 + 1];
						simplified[num42 * 4 + 2] = simplified[(num42 - 1) * 4 + 2];
						simplified[num42 * 4 + 3] = simplified[(num42 - 1) * 4 + 3];
					}
		...
		{
			int num43 = (simplified[l * 4 + 3] + 1) % num11;
			int num44 = simplified[l * 4 + 3];
			simplified[l * 4 + 3] = (verts[num43 * 4 + 3] & 0xFFFF) | (verts[num44 * 4 + 3] & 0x10000);
		}
	}
	...
	public int GetCornerHeight(int x, int z, int i, int dir, ref bool isBorderVertex)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			...
			int num17 = (j + 2) & 3;
			int num18 = (j + 3) & 3;
			bool num19 = (array[num15] & array[num16] & 0x8000u) != 0 && array[num15] == array[num16];
	...
	public void BuildPolyMesh(VoxelContourSet cset, int nvp, out VoxelMesh mesh)
	{
		...
		for (int j = 0; j < cset.conts.Count; j++)
		{
			...
			if (voxelContour.nverts >= 3)
			{
				...
				int num6 = Triangulate(voxelContour.nverts, voxelContour.verts, ref indices, ref tris);
				int num7 = num4;
				for (int l = 0; l < num6 * 3; l++)
				{
					array2[num5] = tris[l] + num7;
					num5++;
				}
		...
		Int3[] array3 = new Int3[num4];
		for (int n = 0; n < num4; n++)
		{
			array3[n] = array[n];
		}
		int[] array4 = new int[num5];
		Buffer.BlockCopy(array2, 0, array4, 0, num5 * 4);
		mesh.verts = array3;
		mesh.tris = array4;
	}
	...
	{
		Vector3 vector = (voxelOffset = forcedBounds.min);
		...
		for (int i = 0; i < inputExtraMeshes.Count; i++)
		{
			...
			{
				num4 = Math.Max(inputExtraMeshes[i].vertices.Length, num4);
			}
		}
		Vector3[] array6 = new Vector3[num4];
		Matrix4x4 matrix4x = Matrix4x4.TRS(-new Vector3(0.5f, 0f, 0.5f), Quaternion.identity, Vector3.one) * Matrix4x4.Scale(new Vector3(num, num2, num)) * Matrix4x4.TRS(-vector, Quaternion.identity, Vector3.one);
		for (int j = 0; j < inputExtraMeshes.Count; j++)
		{
			ExtraMesh extraMesh = inputExtraMeshes[j];
			if (!extraMesh.bounds.Intersects(forcedBounds))
			{
				continue;
			}
			...
			{
				Vector3 vector2 = array6[triangles[l]];
				Vector3 vector3 = array6[triangles[l + 1]];
				Vector3 vector4 = array6[triangles[l + 2]];
				if (flag)
				{
				...
				int value = (int)Utility.Min(vector2.x, vector3.x, vector4.x);
				int value2 = (int)Utility.Min(vector2.z, vector3.z, vector4.z);
				int value3 = (int)Math.Ceiling(Utility.Max(vector2.x, vector3.x, vector4.x));
				int value4 = (int)Math.Ceiling(Utility.Max(vector2.z, vector3.z, vector4.z));
				...
				int area2 = ((!(Vector3.Dot(Vector3.Cross(vector3 - vector2, vector4 - vector2).normalized, Vector3.up) < num3)) ? (1 + area) : 0);
				Utility.CopyVector(array, 0, vector2);
				Utility.CopyVector(array, 3, vector3);
				Utility.CopyVector(array, 6, vector4);
				for (int m = value; m <= value3; m++)
				{
					int num6 = Utility.ClipPolygon(array, 3, array2, 1f, (float)(-m) + 0.5f, 0);
					if (num6 < 3)
					{
						continue;
					}
					num6 = Utility.ClipPolygon(array2, num6, array3, -1f, (float)m + 0.5f, 0);
					if (num6 < 3)
					{
						continue;
					}
					...
					int num10 = Mathf.Clamp((int)Math.Round(num8), 0, voxelArea.depth - 1);
					for (int num11 = num9; num11 <= num10; num11++)
					{
						int num12 = Utility.ClipPolygon(array3, num6, array4, 1f, (float)(-num11) + 0.5f, 2);
						if (num12 < 3)
						{
							continue;
						}
						num12 = Utility.ClipPolygonY(array4, num12, array5, -1f, (float)num11 + 0.5f, 2);
						if (num12 >= 3)
						{
							float num13 = array5[1];
							float num14 = array5[1];
							for (int num15 = 1; num15 < num12; num15++)
							{
								float val2 = array5[num15 * 3 + 1];
								num13 = Math.Min(num13, val2);
								num14 = Math.Max(num14, val2);
							}
							int num16 = (int)Math.Ceiling(num14);
							if (num16 >= 0)
							{
								int num17 = Math.Max(0, (int)num13);
								num16 = Math.Max(num17 + 1, num16);
								voxelArea.AddLinkedSpan(num11 * voxelArea.width + m, (uint)num17, (uint)num16, area2, voxelWalkableClimb);
							}
						}
					}
				}
	...
	{
		int num = voxelArea.compactSpans.Length;
		Vector3[] array = new Vector3[num];
		Vector3[] array2 = new Vector3[num];
		Color[] array3 = new Color[num];
		int num2 = 0;
		int num3 = voxelArea.width * voxelArea.depth;
		Vector3 min = forcedBounds.min;
		int num4 = 0;
		int num5 = 0;
		while (num4 < num3)
		{
			for (int i = 0; i < voxelArea.width; i++)
			{
				Vector3 vector = new Vector3(i, 0f, num5) * cellSize + min;
				CompactVoxelCell compactVoxelCell = voxelArea.compactCells[i + num4];
				int j = (int)compactVoxelCell.index;
				for (int num6 = (int)(compactVoxelCell.index + compactVoxelCell.count); j < num6; j++)
				{
					CompactVoxelSpan compactVoxelSpan = voxelArea.compactSpans[j];
					vector.y = ((float)(int)compactVoxelSpan.y + 0.1f) * cellHeight + min.y;
					array[num2] = vector;
					vector.y = (float)(int)compactVoxelSpan.y * cellHeight + min.y;
					array2[num2] = vector;
			...
			num4 += voxelArea.width;
			num5++;
		}
		DebugUtility.DrawCubes(array, array2, array3, cellSize);
	}
	...
	private void DrawLine(int a, int b, int[] indices, int[] verts, Color col)
	{
		...
		Debug.DrawLine(ConvertPosCorrZ(verts[num], verts[num + 1], verts[num + 2]), ConvertPosCorrZ(verts[num2], verts[num2 + 1], verts[num2 + 2]), col);
	}
