				...
				for (int j = 0; j < actionCategoryIds.Count; j++)
				{
					...
					foreach (InputAction item in ReInput.mapping.UserAssignableActionsInCategory(actionCategory.id, sort: true))
					{
						if (item.type == InputActionType.Axis)
						{
							...
							if (_showSplitAxisInputFields)
							{
								string labelText = ((!string.IsNullOrEmpty(item.positiveDescriptiveName)) ? item.positiveDescriptiveName : (item.descriptiveName + " +"));
								...
								num -= _inputRowHeight;
								string labelText2 = ((!string.IsNullOrEmpty(item.negativeDescriptiveName)) ? item.negativeDescriptiveName : (item.descriptiveName + " -"));
