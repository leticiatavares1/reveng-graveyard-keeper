...
public class UITable : UIWidgetContainer
{
	...
	private static Vector3 RoundVector(Vector3 vec, float value = 0.5f)
	{
		vec.x = Mathf.Round(vec.x / value) * value;
		vec.y = Mathf.Round(vec.y / value) * value;
		vec.z = Mathf.Round(vec.z / value) * value;
		return vec;
	}
	...
	protected void RepositionVariableSize(List<Transform> children)
	{
		...
		for (int count = children.Count; i < count; i++)
		{
			...
			bounds.center = RoundVector(bounds.center);
			bounds.extents = RoundVector(bounds.extents);
