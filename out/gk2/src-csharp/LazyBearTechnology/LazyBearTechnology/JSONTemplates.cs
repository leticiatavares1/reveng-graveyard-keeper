...
public static class JSONTemplates
{
		...
		if (touched.Add(obj))
		{
			...
			foreach (FieldInfo fieldInfo in fields)
			{
				...
				if ((bool)jSONObject)
				{
					...
					Debug.LogWarning("Null for this non-null object, property " + fieldInfo.Name + " of class " + obj.GetType().Name + ". Object type is " + fieldInfo.FieldType.Name);
				}
			}
			...
			foreach (PropertyInfo propertyInfo in properties)
			{
				...
				if ((bool)jSONObject2)
				{
					...
					Debug.LogWarning("Null for this non-null object, property " + propertyInfo.Name + " of class " + obj.GetType().Name + ". Object type is " + propertyInfo.PropertyType.Name);
				}
			}
			return obj2;
		}
	...
	{
		Keyframe result = new Keyframe((float)(obj.HasField("time") ? obj.GetField("time").n : 0.0), (float)(obj.HasField("value") ? obj.GetField("value").n : 0.0));
		if (obj.HasField("inTangent"))
		{
			result.inTangent = (float)obj.GetField("inTangent").n;
		}
		if (obj.HasField("outTangent"))
		{
			result.outTangent = (float)obj.GetField("outTangent").n;
		}
