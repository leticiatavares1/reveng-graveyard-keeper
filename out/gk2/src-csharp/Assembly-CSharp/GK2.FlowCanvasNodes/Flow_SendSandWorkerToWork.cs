...
public class Flow_SendSandWorkerToWork : GKCustomFlowNode
{
	...
	private void DoAction(Flow flow)
	{
		...
		if (selfWgoData.Worker is ZombieWgoData zombieWgoData)
		{
			...
			{
				gDPointData = MainGame.Instance.GameSave.worldData.gdPointsData.GetGDPointDataById($"sand_zombie_{i + 1}");
				if (gDPointData == null)
				{
					break;
				}
			...
			else
			{
				...
				zombieWgoData.MovementComponent.StartPath(gDPointData.Position, graphMask, zombieWgoData.WorldId, 1.5f, "sand_craft_start");
			}
		}
		else
		{
